...
for files in fileList1:
    ...
    del image1
    if os.path.exists(os.path.join(maskDir,f1.replace('jp2\n', 'mat'))):
                    maskB = hdf5storage.loadmat(os.path.join(maskDir,f1.replace('jp2\n', 'mat')))['seg']
    elif os.path.exists(os.path.join(maskDir, 'atlas-seg_to_' + f1.replace('.jp2\n', '_small.geojson'))):
        with open(os.path.join(maskDir, 'atlas-seg_to_' + f1.replace('.jp2\n', '_small.geojson'))) as fAtlas:
    ...
    for row in range(0, org_w - window, window):
        for col in range(0, org_h - window, window):
            ...
            img[:, :, 0] = image[row:row + window, col:col + window, 0]
